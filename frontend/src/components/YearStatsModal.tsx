...
export function YearStatsModal({ history, currentIncome, currentExpenses, onClose }: Props) {
  const allMonths = [
    ...history,
    { mes: 'Este mes', ingresos: currentIncome, gastos: currentExpenses },
  ];
  ...
  return (
    ...
    >
      <div
        className="bg-card sm:border border-border sm:rounded-3xl rounded-t-3xl shadow-2xl max-w-lg w-full overflow-hidden max-h-[80vh] flex flex-col"
        ...
        <div className="overflow-y-auto p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-chart-2/10 rounded-2xl p-4 text-center">
              <p className="text-[10px] text-chart-2 font-bold uppercase tracking-wide">
                Prom. Ingresos
              </p>
            ...
            <div className="bg-chart-4/10 rounded-2xl p-4 text-center">
              <p className="text-[10px] text-chart-4 font-bold uppercase tracking-wide">
                Prom. Gastos
              </p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-right py-2 text-xs text-muted-foreground font-semibold">
                  Ingresos
                </th>
                <th className="text-right py-2 text-xs text-muted-foreground font-semibold">
                  Gastos
              ...
              {allMonths.map((m, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-border/40 last:border-0">
                    <td className={`py-2 text-sm ${i === allMonths.length - 1 ? 'font-bold' : ''}`}>
                      {m.mes}
                    </td>
                    ...
                    <td
                      className={`py-2 text-right font-mono ${bal >= 0 ? 'text-foreground' : 'text-destructive'}`}
                    >
                      {fmt(bal)}
                    </td>
                  </tr>
