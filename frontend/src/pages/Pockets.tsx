  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {monthLabel} · {pockets.length} bolsillos activos
          </p>
        </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-6">
        {pockets.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className={`bg-card border rounded-2xl p-5 shadow-sm transition-all animate-in ${isTransferring ? 'border-accent ring-2 ring-accent/20 shadow-md' : 'border-border hover:shadow-md'}`}
            >
              ...
              {isTransferring ? (
                <div className="border-t border-border pt-3">
                  <div className="flex items-center gap-2 max-w-full overflow-hidden">
                    ...
                    <input
                      autoFocus
                      className="font-mono text-sm flex-1 min-w-0 w-0 border-2 border-accent rounded-xl px-3 py-1.5 bg-card focus:outline-none focus:ring-2 focus:ring-accent/20"
                      placeholder="0"
                ...
                <div className="flex items-center gap-2 border-t border-border pt-3 max-w-full overflow-hidden">
                  ...
                  <input
                    autoFocus
                    className="font-mono text-sm flex-1 min-w-0 w-0 border-2 border-destructive rounded-xl px-3 py-1.5 bg-card focus:outline-none focus:ring-2 focus:ring-destructive/20"
                    placeholder="0"
                    value={withdrawAmount}
                    onChange={(e) => setWithdrawAmount(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') commitWithdraw(p.id);
                      if (e.key === 'Escape') {
                        setWithdrawingFrom(null);
                        setWithdrawAmount('');
                      }
                  ...
                  />
                  <button
                    onClick={() => commitWithdraw(p.id)}
                    className="p-2 rounded-xl bg-destructive text-destructive-foreground hover:bg-destructive/90 transition-colors shrink-0"
                  >
                    <Check className="w-3.5 h-3.5" />
                  </button>
                  <button
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      setTransferringTo(p.id);
                      setTransferAmount('');
                    }}
                    className="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold border border-dashed rounded-xl py-2 transition-colors"
                    style={{ borderColor: p.color, color: p.color }}
                    onMouseEnter={(e) => {
                      (e.currentTarget as HTMLButtonElement).style.backgroundColor = `${p.color}12`;
                    }}
                    onMouseLeave={(e) => {
                  ...
                  <button
                    onClick={() => {
                      setWithdrawingFrom(p.id);
                      setWithdrawAmount('');
                    }}
                    className="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold border border-dashed rounded-xl py-2 transition-colors text-destructive border-destructive/40 hover:bg-destructive/5"
                    style={{}}
                  >
        ...
        <button
          ...
          className="border-2 border-dashed border-border rounded-2xl p-5 flex flex-col items-center justify-center gap-2.5 text-muted-foreground hover:border-primary/50 hover:text-primary hover:bg-primary/5 transition-all min-h-[160px]"
        >
      ...
      <div className="bg-accent/10 border border-accent/20 rounded-2xl p-4 mb-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Al cerrar el mes puedes transferirlo al siguiente mes o distribuirlo entre tus
            bolsillos.
          </p>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-4 mb-6 flex items-center justify-between">
        <div>
          <p className="text-xs text-muted-foreground uppercase tracking-wide font-semibold">
            Total acumulado en bolsillos
          </p>
          <p className="font-mono font-bold text-xl text-foreground mt-0.5">
            {fmt(totalPocketBalance)}
          </p>
        </div>
        <div className="text-right">
          <p className="text-xs text-muted-foreground">Progreso general</p>
          <p className="font-mono font-bold text-lg text-primary">
            {safePercent(totalPocketBalance, totalPocketGoal)}%
          </p>
          <div className="w-32 mt-1">
            <MiniBar value={safePercent(totalPocketBalance, totalPocketGoal)} color="#d4a853" />
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="w-full sm:w-auto flex items-center justify-center gap-2.5 bg-primary text-primary-foreground px-7 py-4 sm:py-3.5 rounded-2xl font-bold text-sm hover:bg-primary/90 active:scale-95 transition-all shadow-lg hover:shadow-xl"
      >
        <Check className="w-4 h-4" /> Cerrar Mes · {monthLabel}
      </button>

      {showCreateModal && (
        <CreatePocketModal onClose={() => setShowCreateModal(false)} onCreated={onPocketsUpdated} />
      )}
